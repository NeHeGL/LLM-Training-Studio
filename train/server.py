...
# This mirrors what start_web.bat / start_app.bat do via PYTHONPYCACHEPREFIX.
...
MODEL_CATALOG = [
    # ── Sub-1B ────────────────────────────────────────────────────────────────
    {
        ...
        "min_vram_4bit": 1,
        "min_vram_fp16": 1,
        "description": "SmolLM2 360M — Apache 2.0, no gating. Smallest practical model for tiny VRAM or rapid testing.",
        "tag": "360M",
    },
    {
        ...
        "min_vram_4bit": 1,
        "min_vram_fp16": 2,
        "description": "Qwen 2.5 0.5B — smallest Qwen, only for very limited VRAM. Limited knowledge.",
        "tag": "0.5B",
    },
    {
        "id": "Qwen/Qwen3-0.6B",
        ...
        "min_vram_4bit": 1,
        "min_vram_fp16": 2,
        "description": "Qwen3 0.6B — Apache 2.0, no gating. Newer architecture than Qwen2.5, hybrid thinking mode.",
        "tag": "0.6B",
    },
    # ── 1–2B ──────────────────────────────────────────────────────────────────
    {
        ...
        "min_vram_4bit": 2,
        "min_vram_fp16": 4,
        "description": "Qwen 2.5 1.5B — slightly more capable than average 1B models.",
        "tag": "1.5B",
    },
    {
        "id": "Qwen/Qwen3-1.7B",
        ...
        "min_vram_4bit": 2,
        "min_vram_fp16": 4,
        "description": "Qwen3 1.7B — Apache 2.0, no gating. Stronger than Qwen2.5-1.5B with hybrid thinking.",
        "tag": "1.7B",
    },
    {
    ...
    },
    {
        ...
        "min_vram_4bit": 2,
        "min_vram_fp16": 4,
        "description": "Pythia 1.4B — Apache 2.0, no gating. Rock-solid GPT-NeoX architecture, works everywhere.",
        "tag": "1.4B",
    },
    # ── 2–4B ──────────────────────────────────────────────────────────────────
    {
        ...
        "min_vram_4bit": 2,
        "min_vram_fp16": 6,
        "description": "Microsoft Phi-2 — MIT license, no gating. Trained on high-quality textbook data. Strong reasoning for its size.",
        "tag": "2.7B",
    },
    {
        ...
        "min_vram_4bit": 3,
        "min_vram_fp16": 8,
        "description": "Microsoft Phi-3.5-mini — MIT license, no gating. 128K context, outperforms many 7B models. Great for fine-tuning.",
        "tag": "3.8B",
    },
    {
        "id": "Qwen/Qwen2.5-3B",
        ...
        "min_vram_4bit": 3,
        "min_vram_fp16": 7,
        "description": "Qwen 2.5 3B — good balance of size and capability for fine-tuning.",
        "tag": "3B",
    },
    {
        "id": "Qwen/Qwen3-4B",
    ...
    # ── 7–9B ──────────────────────────────────────────────────────────────────
    {
        ...
        "min_vram_4bit": 5,
        "min_vram_fp16": 15,
        "description": "Qwen 2.5 7B — recommended for 8–16 GB VRAM. Excellent quality with 4-bit QLoRA.",
        "tag": "7B ⭐",
    },
    {
        "id": "Qwen/Qwen3-8B",
        ...
        "min_vram_4bit": 5,
        "min_vram_fp16": 17,
        "description": "Qwen3 8B — Apache 2.0, no gating. Latest Qwen generation; strong reasoning and instruction following.",
        "tag": "8B ⭐",
    },
    {
        ...
        "min_vram_4bit": 5,
        "min_vram_fp16": 15,
        "description": "OLMo 2 7B — Apache 2.0, no gating, fully open (data + code). Allen AI. Excellent for research fine-tuning.",
        "tag": "7B",
    },
    {
        ...
        "min_vram_4bit": 5,
        "min_vram_fp16": 15,
        "description": "Mistral 7B v0.3 — Apache 2.0, no gating. Improved tokenizer over v0.1. Fast inference, strong instruction following.",
        "tag": "7B",
    },
    {
        ...
        "min_vram_4bit": 5,
        "min_vram_fp16": 15,
        "description": "Mistral 7B v0.1 — Apache 2.0, no gating. Original release; well-tested for fine-tuning.",
        "tag": "7B",
    },
    {
    ...
    # ── 13–14B ────────────────────────────────────────────────────────────────
    {
        ...
        "min_vram_4bit": 8,
        "min_vram_fp16": 27,
        "description": "OLMo 2 13B — Apache 2.0, no gating, fully open. Allen AI. Strong mid-size model for fine-tuning.",
        "tag": "13B",
    },
    {
        "id": "Qwen/Qwen2.5-14B",
        ...
        "min_vram_4bit": 9,
        "min_vram_fp16": 29,
        "description": "Qwen 2.5 14B — excellent for 24 GB cards. Significantly better than 7B.",
        "tag": "14B",
    },
    {
        "id": "Qwen/Qwen3-14B",
    ...
    # ── 32B ───────────────────────────────────────────────────────────────────
    {
        ...
        "min_vram_4bit": 20,
        "min_vram_fp16": 65,
        "description": "Qwen 2.5 32B — near GPT-4 quality. Needs 24+ GB for 4-bit.",
        "tag": "32B",
    },
    {
        "id": "Qwen/Qwen3-32B",
        ...
        "min_vram_4bit": 20,
        "min_vram_fp16": 65,
        "description": "Qwen3 32B — Apache 2.0, no gating. Top open-source quality at 32B. Needs 24+ GB for 4-bit.",
        "tag": "32B",
    },
    # ── 72B ───────────────────────────────────────────────────────────────────
    {
        ...
        "min_vram_4bit": 42,
        "min_vram_fp16": 145,
        "description": "Qwen 2.5 72B — top open-source quality. Multi-GPU or cloud only.",
        "tag": "72B",
    },
    {
        "id": "Qwen/Qwen3-72B",
        ...
        "min_vram_4bit": 42,
        "min_vram_fp16": 145,
        "description": "Qwen3 72B — Apache 2.0, no gating. State-of-the-art open model. Multi-GPU or cloud only.",
        "tag": "72B",
    },
]
...
def create_app():
    ...
    def model_catalog():
        ...
        if vram >= 48:
            recommended_base = "Qwen/Qwen3-32B"
        elif vram >= 24:
            recommended_base = "Qwen/Qwen3-14B"
        elif vram >= 16:
            recommended_base = "Qwen/Qwen3-8B"
        elif vram >= 10:
            recommended_base = "Qwen/Qwen3-4B"
        elif vram >= 6:
            recommended_base = "microsoft/Phi-3.5-mini-instruct"
        elif vram > 0:
    ...
    def system_info():
        try:
            ...
            if vram >= 48:
                info["recommended_base"] = "Qwen/Qwen3-32B"
                info["recommended_alts"] = [
                    "Qwen/Qwen3-72B",
                    "Qwen/Qwen2.5-32B",
                ]
            elif vram >= 24:
                info["recommended_base"] = "Qwen/Qwen3-14B"
                info["recommended_alts"] = [
                    "Qwen/Qwen3-8B",
                    "Qwen/Qwen2.5-14B",
                ]
            elif vram >= 16:
                info["recommended_base"] = "Qwen/Qwen3-8B"
                info["recommended_alts"] = [
                    "Qwen/Qwen2.5-7B",
                    "mistralai/Mistral-7B-v0.3",
                ]
            elif vram >= 10:
                info["recommended_base"] = "Qwen/Qwen3-4B"
                info["recommended_alts"] = [
                    "microsoft/Phi-3.5-mini-instruct",
                    "Qwen/Qwen2.5-3B",
                ]
            elif vram >= 6:
                info["recommended_base"] = "microsoft/Phi-3.5-mini-instruct"
                info["recommended_alts"] = [
                    "Qwen/Qwen3-1.7B",
                    "microsoft/phi-2",
                ]
            elif vram > 0:
                info["recommended_base"] = "Qwen/Qwen3-1.7B"
    ...
    def list_packages():
        ...
        try:
            ...
            # ── Torch CUDA false-positive fix ──────────────────────────────────
            # pip list --outdated compares against PyPI which only has CPU wheels.
            # If torch is installed with a CUDA suffix (e.g. 2.11.0+cu128) we must
            # check the CUDA wheel index instead and remove the PyPI false-positive.
            import re as _re2
            torch_installed = installed.get("torch", "")
            _cu_match = _re2.search(r'\+(cu\d+)', torch_installed)
            if _cu_match and "torch" in outdated:
                _cu_tag  = _cu_match.group(1)          # e.g. "cu128"
                _idx_url = f"https://download.pytorch.org/whl/{_cu_tag}"
                try:
                    _idx_res = subprocess.run(
                        [sys.executable, "-m", "pip", "index", "versions", "torch",
                         "--index-url", _idx_url],
                        capture_output=True, text=True, timeout=30,
                    )
                    # Output format: "torch (2.11.0+cu128)\n  AVAILABLE VERSIONS: 2.11.0+cu128, ..."
                    _versions_line = ""
                    for _ln in _idx_res.stdout.splitlines():
                        if "AVAILABLE" in _ln.upper() or _versions_line:
                            _versions_line += _ln
                    # Also parse the first "torch (X.Y.Z+cuNNN)" line for the latest
                    _latest_cuda_match = _re2.search(
                        r'torch\s*\(([0-9]+\.[0-9]+\.[0-9]+\+' + _cu_tag + r')\)',
                        _idx_res.stdout
                    )
                    if _latest_cuda_match:
                        _latest_cuda_ver = _latest_cuda_match.group(1)  # e.g. "2.11.0+cu128"
                        # Strip the +cuNNN suffix for comparison with installed base version
                        _installed_base = torch_installed.split("+")[0]
                        _latest_base    = _latest_cuda_ver.split("+")[0]
                        if _installed_base >= _latest_base:
                            # CUDA channel has nothing newer — remove the PyPI false-positive
                            del outdated["torch"]
                        else:
                            # CUDA channel genuinely has a newer version — show that one
                            outdated["torch"] = _latest_cuda_ver
                    else:
                        # Could not parse CUDA channel latest — be safe, remove false-positive
                        del outdated["torch"]
                except Exception:
                    # Network error etc. — remove the PyPI false-positive to avoid confusion
                    del outdated["torch"]
            # ───────────────────────────────────────────────────────────────────
    ...
    def stream_update():
        ...
        def generate():
            ...
            if torch_pkgs:
                ...
                # If no index URL was supplied by the caller, auto-detect from the
                # currently installed torch wheel's local version tag (e.g. +cu128).
                # Using torch.version.cuda (the CUDA runtime torch was compiled against)
                # is NOT reliable — it may differ from the wheel channel tag.
                # Instead, read the installed version string from pip and extract
                # the +cuXXX suffix directly (e.g. "2.11.0+cu128" → "cu128").
                # This guarantees we use the same PyTorch index the user originally
                # installed from, avoiding "Requirement already satisfied" no-ops.
                _idx = torch_index
                if not _idx:
                    try:
                        import subprocess as _subp, re as _re2
                        _pip_out = _subp.run(
                            [sys.executable, "-m", "pip", "show", "torch"],
                            capture_output=True, text=True, timeout=10,
                        )
                        # Version line looks like: "Version: 2.11.0+cu128"
                        _ver_match = _re2.search(r'Version:\s*[\d.]+\+(cu\d+)', _pip_out.stdout)
                        if _ver_match:
                            _cu_tag = _ver_match.group(1)   # e.g. "cu128"
                            _idx = f"https://download.pytorch.org/whl/{_cu_tag}"
                            yield f"data: {_json.dumps({'type': 'line', 'text': f'[torch] Detected wheel tag {_cu_tag} — using index: {_idx}'})}\n\n"
                        else:
                            # No +cuXXX suffix → CPU-only or unknown; use PyPI
                            yield f"data: {_json.dumps({'type': 'line', 'text': '[torch] No CUDA wheel tag found — upgrading from PyPI (CPU-only)'})}\n\n"
                    except Exception as _ex:
                        yield f"data: {_json.dumps({'type': 'line', 'text': f'[torch] Could not detect wheel tag ({_ex}) — falling back to PyPI'})}\n\n"
                if _idx:
                    cmd += ["--index-url", _idx]
    ...
    def restart_server():
        ...
        Standalone mode (start_web.bat / python train/server.py):
        ...
        Desktop/embedded mode (start_app.bat / launch_app.py):
...
def _is_our_server_running(port):
    """Return True if LLM Training Studio is already serving on *port*.

    Sends a lightweight GET / request and checks the response for known Studio
    fingerprints.  Returns False on any error so startup is never blocked by a
    transient network issue.
    """
    try:
        import urllib.request as _ur
        resp = _ur.urlopen(f"http://localhost:{port}", timeout=2)
        body = resp.read(512).decode("utf-8", errors="ignore")
        # studio.html contains "LLM Training Studio" in the title
        return "LLM Training Studio" in body or "studio" in body.lower()
    except Exception:
        return False
...
def main():
    ...
    # If our server is already running on this port, don't start a second instance.
    # This prevents double-server when start_app.bat or start_web.bat is run
    # while a server is already active (e.g. both bat files used simultaneously).
    if _is_our_server_running(args.port):
        print(f"[studio] Server already running on port {args.port} — not starting a second instance.")
        if args.open_browser:
            webbrowser.open(f"http://localhost:{args.port}")
        return

    # Kill any leftover *stale* processes on this port (crashed/zombie servers)
